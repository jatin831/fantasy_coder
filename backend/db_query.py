...
def checkUser(user):
	try:
		connection = psycopg2.connect(user="postgres",
			...
			port="5432")
		cursor = connection.cursor()
		cursor.execute("SELECT username,email_id,password FROM users WHERE email_id=%s OR username=%s AND password=crypt(%s, password)",(user['email_id'],user['username'],user['password']))
